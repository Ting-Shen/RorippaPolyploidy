...
resfn = "DIVALIKE+t12+t21+m2_inf_PL.Rdata"
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
resfn = "DIVALIKEJ+t12+t21+m2_inf_PL.Rdata"
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
jstart = 0.0001
xstart = resDIVALIKE_t12_t21_m2$outputs@params_table["x","est"]
...
BioGeoBEARS_run_object$BioGeoBEARS_model_object@params_table["x","type"] = "fixed"
...
resfn = "DIVALIKEJ+t12+t21+m2_rep2_inf_PL.Rdata"
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
outfn = slashslash("divalike_params_inferred_PL.txt")
...
for (i in 1:length(m2_vals))
{
  ...
  BioGeoBEARS_run_object = resDIVALIKE_t12_t21_m2$inputs
...
write.table(lnL_table, file="LINEAGES_m2_DIVALIKE_vs_lnL_table_v1_PL.txt", sep="\t", quote=FALSE)

# Confidence interval
likelihood = exp(resDIVALIKE_t12_t21_m2$total_loglikelihood)
lower95_likelihood = (1/20) * likelihood
lower95_lnL = log(lower95_likelihood)

pdffn = "LINEAGES_m2_DIVALIKE_vs_lnL_v2.pdf"
...
DIVALIKE_95_table = lnL_table[lnL_table$lnLs > lower95_lnL, ]
DIVALIKE_95_table = subset(DIVALIKE_95_table, select = -c(ivals))
DIVALIKE_95_table$likelihoods = exp(DIVALIKE_95_table$lnLs)
DIVALIKE_95_table$model = "DIVALIKE"
DIVALIKE_95_table$slikelihoods = DIVALIKE_95_table$likelihoods-lower95_likelihood
DIVALIKE_95_table$AICc = 0.0737
write.table(DIVALIKE_95_table, file = "DIVALIKE_95_table_PL.txt", sep="\t", quote=FALSE)


# 95% CI
m2_ML_est = resDIVALIKE_t12_t21_m2$outputs@params_table["m2","est"]
...
for (i in 1:length(m2_vals))
{
  ...
  BioGeoBEARS_run_object = resDIVALIKEj_t12_t21_m2$inputs
  BioGeoBEARS_run_object$BioGeoBEARS_model_object@params_table["m2","max"] = 100.0
  BioGeoBEARS_run_object$BioGeoBEARS_model_object@params_table["m2","init"] = m2_val
  BioGeoBEARS_run_object$BioGeoBEARS_model_object@params_table["m2","est"] = m2_val
  lnL = bears_optim_run(BioGeoBEARS_run_object, skip_optim=TRUE, skip_optim_option="return_loglike")
...
lnL_table = cbind(ivals, m2_vals, lnLs)
lnL_table = adf2(lnL_table)
write.table(lnL_table, file="LINEAGES_m2_DIVALIKEj_vs_lnL_table_v1_PL.txt", sep="\t", quote=FALSE)

# Confidence interval
likelihood = exp(resDIVALIKEj_t12_t21_m2$total_loglikelihood)
lower95_likelihood = (1/20) * likelihood
lower95_lnL = log(lower95_likelihood)

pdffn = "LINEAGES_m2_DIVALIKEj_vs_lnL_v2_PL.pdf"
pdf(file=pdffn, width=9, height=12)
...
#make table with m2 values with less than <5% removed
DIVALIKEj_95_table = lnL_table[lnL_table$lnLs > lower95_lnL, ]
DIVALIKEj_95_table = subset(DIVALIKEj_95_table, select = -c(ivals))
DIVALIKEj_95_table$likelihoods = exp(DIVALIKEj_95_table$lnLs)
DIVALIKEj_95_table$model = "DIVALIKE+J"
DIVALIKEj_95_table$slikelihoods = DIVALIKEj_95_table$likelihoods-lower95_likelihood
DIVALIKEj_95_table$AICc = 18.2188
write.table(DIVALIKEj_95_table, file = "DIVALIKEj_95_table_PL.txt", sep="\t", quote=FALSE)


# 95% CI
m2_ML_est = resDIVALIKEj_t12_t21_m2$outputs@params_table["m2","est"]
